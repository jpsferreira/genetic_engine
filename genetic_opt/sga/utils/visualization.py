...
def plot_metrics(
    ...
    if "best_fitness" in metrics_df.columns and "avg_fitness" in metrics_df.columns:
        axs[0, 0].plot(
            ...
            metrics_df["best_fitness"],
            "b-",
            label="Best Fitness",
        )
        axs[0, 0].plot(
            metrics_df["generation"],
            metrics_df["avg_fitness"],
            "g--",
            label="Avg Fitness",
        )
...
def plot_population_density(
    ...
    if generations is None:
        ...
        if n_plots > 1:
            selected_gens = [
                unique_gens[i]
                for i in np.linspace(0, len(unique_gens) - 1, n_plots).astype(int)
            ]
        else:
    ...
    fig.suptitle(
        f"Population Density Evolution (Genes {gene_indices[0]+1} & {gene_indices[1]+1})",
        fontsize=16,
    )
...
def create_population_migration_animation(
    ...
    if output_file:
        ...
        if output_file_str.endswith(".gif"):
            anim.save(output_file_str, writer="pillow", fps=fps, dpi=100)
        elif output_file_str.endswith(".mp4"):
            anim.save(output_file_str, writer="ffmpeg", fps=fps, dpi=200)
        else:
...
def plot_population_statistics(
    ...
    if metrics_file and Path(metrics_file).exists():
        ...
        axs[1, 0].plot(
            stats_df["generation"], [sum(stats["gene_stds"]) for stats in stats_by_gen]
        )
...
def analyze_population_migration(
    ...
    if run_dir:
        ...
        if (
            len(timestamp_parts) >= 3
            and timestamp_parts[-2].isdigit()
            and timestamp_parts[-1].isdigit()
        ):
    ...
    metrics_file_path = population_path.parent / Path(
        base_filename.replace("_population", "_metrics")
    ).with_suffix(".csv")
    ...
    for gene_pair in gene_pairs:
        ...
        density_fig = plot_population_density(
            population_file, gene_indices=[i, j], output_file=density_file
        )
        ...
        if create_animation:
            anim_file = (
                analysis_dir / f"{base_filename}_animation_genes_{i+1}_{j+1}.mp4"
            )
            anim = create_population_migration_animation(
                population_file, gene_indices=[i, j], output_file=anim_file
            )
        ...
        if fitness_function is not None:
            ...
            landscape_file = (
                analysis_dir
                / f"{base_filename}_fitness_landscape_genes_{i+1}_{j+1}.png"
            )
            ...
            if create_animation:
                landscape_anim_file = (
                    analysis_dir
                    / f"{base_filename}_fitness_landscape_animation_genes_{i+1}_{j+1}.mp4"
                )
                ...
                results["fitness_animations"][f"genes_{i+1}_{j+1}"] = str(
                    landscape_anim_file
                )
    ...
    if include_correlations:
        ...
        corr_fig = plot_pairwise_correlations(population_file, output_file=corr_file)
...
def plot_fitness_landscape(
    ...
    fig, axs = plt.subplots(n_rows, n_cols, figsize=figsize, squeeze=False)
    fig.suptitle(
        f"Fitness Landscape and Population Evolution (Genes {gene_indices[0]+1} & {gene_indices[1]+1})",
        fontsize=16,
    )
...
def create_fitness_landscape_animation(
    ...
    scatter = ax.scatter(
        [], [], c=population_color, s=20, alpha=alpha, edgecolor="white", linewidth=0.5
    )
    ...
    def update(frame):
        ...
        scatter.set_offsets(
            np.column_stack((gen_data[gene_cols[0]], gen_data[gene_cols[1]]))
        )
...
def plot_3d_migration_trajectory(
    ...
    if len(gene_indices) != 3:
        raise ValueError(
            "Exactly three gene indices must be provided for 3D visualization"
        )
    ...
    if color_by == "generation":
        ...
        scatter = ax.scatter(
            x, y, z, c=generations, cmap="viridis", s=marker_size, alpha=0.6
        )
        ...
        # Plot population scatter
        scatter = ax.scatter(
            x, y, z, c=density, cmap="plasma", s=marker_size, alpha=0.6
        )
    ...
    if density_plot:
        ...
        try:
            ...
            resolution = 50
            xx_xy, yy_xy = np.mgrid[
                x_min : x_max : resolution * 1j, y_min : y_max : resolution * 1j
            ]
            xx_xz, zz_xz = np.mgrid[
                x_min : x_max : resolution * 1j, z_min : z_max : resolution * 1j
            ]
            yy_yz, zz_yz = np.mgrid[
                y_min : y_max : resolution * 1j, z_min : z_max : resolution * 1j
            ]
...
def plot_reduced_space_migration(
    population_data,
    output_dir,
    output_prefix,
    method="pca",
    ...
    if len(gene_columns) < n_components:
        print(
            f"Warning: Not enough gene dimensions ({len(gene_columns)}) for {n_components} components."
        )
        return {}
    ...
    if n_components == 2:
        ...
        if explained_variance is not None:
            plt.xlabel(
                f"{method_name} Component 1 ({explained_variance[0]:.2%} variance)"
            )
            plt.ylabel(
                f"{method_name} Component 2 ({explained_variance[1]:.2%} variance)"
            )
    else:
        ...
        if explained_variance is not None:
            ...
            ax.set_zlabel(
                f"{method_name} Component 3 ({explained_variance[2]:.2%} variance)"
            )
    ...
    if create_animation:
        ...
        if n_components == 2:
            ...
            margin = 0.1
            x_min, x_max = (
                population_data[f"{method.lower()}_component_1"].min(),
                population_data[f"{method.lower()}_component_1"].max(),
            )
            y_min, y_max = (
                population_data[f"{method.lower()}_component_2"].min(),
                population_data[f"{method.lower()}_component_2"].max(),
            )
            x_range = x_max - x_min
            y_range = y_max - y_min
            ...
            def update(frame):
                ...
                gen_data = population_data[
                    population_data["generation"] == generations[frame]
                ]
                ...
                if len(gen_data) > n_clusters:
                    kmeans_gen = KMeans(
                        n_clusters=min(n_clusters, len(gen_data) // 2),
                        random_state=42,
                        n_init=10,
                    )
                    gen_clusters = kmeans_gen.fit_predict(
                        gen_data[
                            [
                                f"{method.lower()}_component_1",
                                f"{method.lower()}_component_2",
            ...
            z_min, z_max = (
                population_data[f"{method.lower()}_component_3"].min(),
                population_data[f"{method.lower()}_component_3"].max(),
            )
            x_range = x_max - x_min
            y_range = y_max - y_min
            z_range = z_max - z_min
        ...
        # Create animation
        ani = FuncAnimation(
            fig, update, frames=len(generations), init_func=init, blit=True
        )
