...
class OptimizationMonitor:
    ...
    def _draw_progress_bar(self) -> None:
        ...
        if bar_width > filled_width and self.width > 12 + filled_width:
            unfilled_str = "░" * min(
                bar_width - filled_width, self.width - 12 - filled_width - 1
            )
        ...
        if self.width > 13 + bar_width:
            self.stdscr.addstr(
                2, min(13 + bar_width, self.width - len(percent) - 1), percent
            )
    ...
    def _draw_metrics(self) -> None:
        ...
        if self.height >= 5:
            fitness_y = 4
            self._safe_addstr(
                fitness_y, 2, "FITNESS METRICS", curses.color_pair(5) | curses.A_BOLD
            )
            ...
            if "best_fitness" in self.metrics and self.metrics["best_fitness"]:
                ...
                if fitness_y + 1 < self.height:
                    ...
                    self._safe_addstr(
                        fitness_y + 1, 17, f"{best_fitness:.6f}", fitness_color
                    )

                if fitness_y + 2 < self.height:
                    self._safe_addstr(
                        fitness_y + 2, 4, "Population Avg: ", curses.A_BOLD
                    )
                    self._safe_addstr(fitness_y + 2, 19, f"{avg_fitness:.6f}")

                if fitness_y + 3 < self.height:
                    self._safe_addstr(
                        fitness_y + 3, 4, "Population Std: ", curses.A_BOLD
                    )
        ...
        if self.height >= 16:
            perf_y = 14
            self._safe_addstr(
                perf_y, 2, "PERFORMANCE METRICS", curses.color_pair(5) | curses.A_BOLD
            )
            ...
            if (
                "memory_usage_mb" in self.metrics
                and self.metrics["memory_usage_mb"]
                and self.width >= 60
            ):
                ...
                if perf_y + 1 < self.height:
                    self._safe_addstr(
                        perf_y + 1,
                        min(40, self.width - 20),
                        "Memory Usage: ",
                        curses.A_BOLD,
                    )
                    self._safe_addstr(
                        perf_y + 1, min(53, self.width - 10), f"{memory:.2f} MB"
                    )
                ...
                if self.height >= 22 and self.width >= 85:
                    self._draw_mini_chart(
                        18,
                        min(40, self.width - 45),
                        "Memory Usage",
                        "memory_usage_mb",
                        min(40, self.width - 50),
                    )
    ...
    def _draw_mini_chart(
        self, y: int, x: int, title: str, metric_key: str, width: int
    ) -> None:
        ...
        if (
            x + width + 2 < self.width - len(min_text)
            and y + 1 + chart_height < self.height
        ):
