    ...
    if args.enhanced_viz and not args.no_track and not args.no_analysis:
        ...
        def run_with_enhanced_viz():
            ...
            if analysis_results.get("3d_visualization"):
                print(
                    f"- 3D migration visualization: {analysis_results['3d_visualization']}"
                )

            if analysis_results.get("correlation_plots"):
                print(
                    f"- Gene correlation analysis: {analysis_results['correlation_plots']}"
                )
                ...
                if analysis_results.get("reduced_space_plots"):
                    for key, path in analysis_results["reduced_space_plots"].items():
                        if key == "reduced_space_plot":
                            print(
                                f"- Dimensionality reduction ({dim_reduction_method.upper()}) visualization: {path}"
                            )

                if (
                    analysis_results.get("reduced_space_plots")
                    and "reduced_space_animation"
                    in analysis_results["reduced_space_plots"]
                ):
                    print(
                        f"- Dimensionality reduction animation: {analysis_results['reduced_space_plots']['reduced_space_animation']}"
                    )
